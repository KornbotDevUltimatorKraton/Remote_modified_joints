...
for i in count(0):
  ...
  for fl in range(0,len(frame_list)-1):
      ...
      if fl == 0:
          ...
          req_feedback = requests.post('https://roboreactor.com/feedback_sensor',json={'teslacoil358@gmail.com':{'BD3':frame_list[fl]}})
          #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
          ...
          for poons_num  in range(0,24):
                  ...
                  if poons_num <= 6:   
                             ...
                             for cr in range(0,8):
                                     ...
                                     #Backward arm to the defaul positioning 
                                     for base_angle in range(167,int(default_arm['IRB1']['Analog-read']),-1):
                                                        frame_list[fl]['IRB1']['Analog-read'] = base_angle 
                                                        req_feedback = requests.post('https://roboreactor.com/feedback_sensor',json={'teslacoil358@gmail.com':{'BD3':frame_list[fl]}})  
                                     for se_angle in range(90,int(default_arm['IRB2']['Analog-read'])):
                                                        print("Control the Shoulder and Elbow joint angle: ",se_angle) 
                                                        frame_list[fl]['IRB2']['Analog-read'] = se_angle 
                                                        frame_list[fl]['IRB3']['Analog-read'] = se_angle 
                                                        req_feedback = requests.post('https://roboreactor.com/feedback_sensor',json={'teslacoil358@gmail.com':{'BD3':frame_list[fl]}})   
                                     frame_list[fl]['IRB3']['Analog-read'] = default_arm['IRB3']['Analog-read']                                
                                     ...
                                     for ce in calibrating_joint:
                                           ...
                                           if frame_list[fl][joint_name]['Analog-read'] > default_arm[joint_name]['Analog-read']:
                                                       print("Default joint angle more than target joint angle ",joint_name)
                                                       for angle_control in range(int(frame_list[fl][joint_name]['Analog-read']),int(default_arm[joint_name]['Analog-read']),-1):
                                                                print("Now control max min: ",joint_name,angle_control)   
                                                                frame_list[fl][joint_name]['Analog-read'] = angle_control
                                                                req_feedback = requests.post('https://roboreactor.com/feedback_sensor',json={'teslacoil358@gmail.com':{'BD3':frame_list[fl]}})
                                           ...
                                           if frame_list[fl][joint_name]['Analog-read'] < default_arm[joint_name]['Analog-read']:
                                                       print("Default joint angle lesser than target joint angle ",joint_name) 
                                                       for angle_control in range(int(frame_list[fl][joint_name]['Analog-read']),int(default_arm[joint_name]['Analog-read'])):
                                                                print("Now control min max: ",joint_name,angle_control)
                                                                frame_list[fl][joint_name]['Analog-read'] = angle_control
                                                                req_feedback = requests.post('https://roboreactor.com/feedback_sensor',json={'teslacoil358@gmail.com':{'BD3':frame_list[fl]}})
                                           ...
                                           if frame_list[fl][joint_name]['Analog-read'] == default_arm[joint_name]['Analog-read']:
                                                       print("Default joint angle is equal to the target joint angle ",joint_name)
                                                       print("Adding the default joint angle data ",default_arm[joint_name]['Analog-read']) 
                                                       frame_list[fl][joint_name]['Analog-read'] = default_arm[joint_name]['Analog-read'] 
                                                       req_feedback = requests.post('https://roboreactor.com/feedback_sensor',json={'teslacoil358@gmail.com':{'BD3':frame_list[fl]}})  
                  ...
                  if poons_num <=12: 
                             ...
                             for ce in range(0,6):
                                     ...
                                     frame_list[fl]['POONZ6']['Analog-read'] = 0 
                                     #Running the last pallett to send to the packaging machine 
                                     print("Current poon 2 payload: ",frame_list[fl][package_poons[2]]['Analog-read'],frame_list[fl][poon_payload[1]]['Analog-read']-7)
                                     ...
                                     if ce == 6:
                                              frame_list[fl][package_poons[2]]['Analog-read'] = 0                     
                                     #Backward arm to the defaul positioning 
                                     for base_angle in range(167,int(default_arm['IRB1']['Analog-read']),-1):
                                                        frame_list[fl]['IRB1']['Analog-read'] = base_angle 
                                                        req_feedback = requests.post('https://roboreactor.com/feedback_sensor',json={'teslacoil358@gmail.com':{'BD3':frame_list[fl]}})  
                  ...
                  if poons_num <= 18:
                             ...
                             for cr in range(0,6):
                                     ...
                                     if ce == 6:
                                            frame_list[fl][package_poons[3]]['Analog-read'] = 0      
                                     #Backward arm to the defaul positioning 
                                     for base_angle in range(167,int(default_arm['IRB1']['Analog-read']),-1):
                                                        frame_list[fl]['IRB1']['Analog-read'] = base_angle 
                                                        req_feedback = requests.post('https://roboreactor.com/feedback_sensor',json={'teslacoil358@gmail.com':{'BD3':frame_list[fl]}})  
                  ...
                  if poons_num <=24:
                             ...
                             for ce in range(0,6):
                                     ...
                                     if ce == 6: 
                                             frame_list[fl][package_poons[1]]['Analog-read'] = 0                
                                     #Backward arm to the defaul positioning 
                                     for base_angle in range(167,int(default_arm['IRB1']['Analog-read']),-1):
                                                        frame_list[fl]['IRB1']['Analog-read'] = base_angle 
                                                        req_feedback = requests.post('https://roboreactor.com/feedback_sensor',json={'teslacoil358@gmail.com':{'BD3':frame_list[fl]}})  
                  ...
                  if len(store_complete) == 4:
                                     ''' 
                                     frame_list[fl]['PalletZ2']['Analog-read'] = 50
                                     frame_list[fl]['PalletZ1']['Analog-read'] = 500
                                     req_feedback = requests.post('https://roboreactor.com/feedback_sensor',json={'teslacoil358@gmail.com':{'BD3':frame_list[fl]}})
                                     '''  
                                     #Activate the pallett slider to the packaging 

                                     for convy3 in range(frame_list[fl]['PallettX3']['Analog-read'],0,-1):
                                             print("Converyer 3 slider: ",convy3) #Conveyer 3 position control slider 
                                             frame_list[fl]['PallettX3']['Analog-read'] = convy3 
                                             req_feedback = requests.post('https://roboreactor.com/feedback_sensor',json={'teslacoil358@gmail.com':{'BD3':frame_list[fl]}})  
                  frame_list[fl]['PallettZ4']['Analog-read']  = 0                        
                  frame_list[fl]['PallettX4']['Analog-read'] = 100   
                  req_feedback = requests.post('https://roboreactor.com/feedback_sensor',json={'teslacoil358@gmail.com':{'BD3':frame_list[fl]}})  
                  #End of process                  
          #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>